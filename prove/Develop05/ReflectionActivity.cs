...
public class ReflectionActivity : Activity
{
    ...
    public void Run()
    {
        ...
        int questionPauseSeconds = 10; // Pause 10s after each question

        while (DateTime.Now < endTime)
        {
            string question = GetRandomQuestion();
            Console.Write($"> {question} ");
            ShowSpinner(questionPauseSeconds); // Pause for reflection
            Console.WriteLine(); // New line for next question

            // If the spinner itself took too long, check end time again
